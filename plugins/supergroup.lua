...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return '<i>Link posting is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Link posting has been locked</i>'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '<i>Link posting is not locked</i>'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Link posting has been unlocked</i>'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return '<i>SuperGroup spam is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>SuperGroup spam has been locked</i>'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '<i>SuperGroup spam is not locked</i>'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>SuperGroup spam has been unlocked</i>'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return '<i>Flood has been locked</i>'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return '<i>Flood is not locked</i>'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Flood has been unlocked</i>'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return '<i>Arabic is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Arabic has been locked</i>'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '<i>Arabic/Persian is already unlocked</i>'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Arabic/Persian has been unlocked</i>'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return '<i>SuperGroup members are already locked</i>'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return '<i>SuperGroup members has been locked</i>'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '<i>SuperGroup members are not locked</i>'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>SuperGroup members has been unlocked</i>'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return '<i>RTL is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>RTL has been locked</i>'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return '<i>RTL is already unlocked</i>'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>RTL has been unlocked</i>'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return '<i>Tgservice is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Tgservice has been locked</i>'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return '<i>TgService Is Not Locked!</i>'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Tgservice has been unlocked</i>'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return '<i>Sticker posting is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Sticker posting has been locked</i>'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return '<i>Sticker posting is already unlocked</i>'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Sticker posting has been unlocked</i>'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return '<i>Contact posting is already locked</i>'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Contact posting has been locked</i>'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return '<i>Contact posting is already unlocked</i>'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Contact posting has been unlocked</i>'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return '<i>Settings are already strictly enforced</i>'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<i>Settings will be strictly enforced</i>'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return '<i>Settings are not strictly enforced</i>'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return '<i>Settings will not be strictly enforced</i>'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return '<b>SuperGroup</b> <code>rules</code> <i>set</i>'
end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "<code>You can't kick mods/owner/admins</code>")
    end
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "<code>You can't kick other admins</code>")
    end
	...
	elseif get_cmd == "setadmin" then
		...
		if result.from.username then
			text = "@"..result.from.username.." <i>set as an admin</i>"
		else
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] <code>added as owner</code>"
			else
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] <code>removed from the muted user list</code")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] <code>added to the muted user list</code>")
		end
	end
end
...
local function set_supergroup_photo(msg, success, result)
  ...
  if success then
    ...
    save_data(_config.moderation.data, data)
    send_large_msg(receiver, '<code>Photo saved!</code>', ok_cb, false)
  else
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "<b>SuperGroup</b> <code>ID</code> <i>for</i> " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
			end
		end
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "<b>Create a link using</b> /newlink <b>first!</b>\n\n<i>Or if I am not creator use</i> /setlink <i>to set your link</i>"
			end
		...
		if matches[1] == "setabout" and is_momod(msg) then
			...
			return "<code>Description has been set.\n\nSelect the chat again to see the changes.</code>"
		end
		...
		if matches[1] == 'clean' then
			...
			if matches[2] == 'modlist' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return '<code>No moderator(s) in this SuperGroup.</code>'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
				...
				return '<i>Modlist has been cleaned</i>'
			end
			if matches[2] == 'rules' then
				...
				return '<i>Rules have been cleaned</i>'
			end
			if matches[2] == 'about' then
				...
				channel_set_about(receiver, about_text, ok_cb, false)
				return "<i>About has been cleaned</i>"
			end
			if matches[2] == 'mutelist' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "<i>Mutelist Cleaned</i>"
			end
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "<code>Message /superhelp to @SudoRobot in private for SuperGroup help</code>"
